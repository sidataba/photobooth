...
import { KOREAN_SCENES, ColorGrade } from './constants';
...
export default function App() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Calculate preview scale to keep performance high (max 1280px width for preview)
        const maxDim = 1280;
  ...
  // Initialize canvas and capture original image data when image is loaded
  useEffect(() => {
    if (image && canvasRef.current && !originalImageData) {
      const canvas = canvasRef.current;
      const ctx = canvas.getContext('2d', { willReadFrequently: true });
      if (ctx) {
        // Preview canvas size
        canvas.width = image.width * previewScale;
        canvas.height = image.height * previewScale;
  ...
  }, [image, originalImageData, previewScale]);
  ...
  const handleReset = () => {
    if (image && canvasRef.current) {
      ...
      if (ctx) {
        ...
        // We don't need to clear originalImageData, just re-apply if needed
      }
    }
  };
  ...
  }, [originalImageData, selectedScene]);
  ...
  }, [selectedScene, intensity, skinRetouch, grain, vignette, temperature, tint, distortion, letterbox, sharpen, originalImageData, applyFilter]);
